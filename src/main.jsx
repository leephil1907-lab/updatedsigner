...
import ExecutionGraph from'./reactbits/ExecutionGraph.jsx';
...
async function startObjective(text,setEvents,setGraph,setResult){
  ...
  setEvents(['Objective '+started.id+' · queued']);
  let last='';
  for(let i=0;i<240;i++){
    const run=await api('/api/objectives/'+started.id);
    setGraph(run.stages||[]);
    const changed=(run.stages||[]).map(s=>s.id+':'+s.status+':'+(s.error||'')).join('|');
    if(changed!==last){
      last=changed;
      setEvents((run.stages||[]).filter(s=>s.status==='running'||s.status==='completed'||s.status==='blocked').map(s=>s.label+' · '+s.status+(s.error?': '+s.error:'')));
    }
    if(['completed','partial','blocked','failed'].includes(run.status)){setResult(run);return run}
    await new Promise(r=>setTimeout(r,1000));
  }
  throw Error('Objective execution polling timed out.');
}

function App(){
 const[agents,setAgents]=useState([]),[agent,setAgent]=useState('command'),[objective,setObjective]=useState(''),[busy,setBusy]=useState(false),[result,setResult]=useState(null),[graph,setGraph]=useState([]),[events,setEvents]=useState([]),[runs,setRuns]=useState([]),[objectiveRuns,setObjectiveRuns]=useState([]),[radar,setRadar]=useState([]),[caps,setCaps]=useState([]),[wow,setWow]=useState(null),[view,setView]=useState('command'),[toast,setToast]=useState(''),[palette,setPalette]=useState(false),inputRef=useRef(null);
 ...
 const loadObjectives=async()=>{try{const d=await api('/api/objectives');setObjectiveRuns(d.items||[])}catch{setObjectiveRuns([])}};
 const loadWow=async()=>{try{setWow(await api('/api/wow/status'))}catch(e){setWow({connected:false,message:e.message})}};
 useEffect(()=>{refresh();loadRuns();loadCaps();loadObjectives()},[]);
 useEffect(()=>{if(view==='radar')loadRadar();if(view==='runs')loadRuns();if(view==='receipts'){loadWow();loadObjectives()}if(view==='capabilities')loadCaps()},[view]);
 const execute=async()=>{const text=objective.trim();if(!text){notify('Enter an objective first.');inputRef.current?.focus();return}setBusy(true);setResult(null);setEvents([]);
  try{let d;
   ...
   }setResult(d);setGraph([]);setEvents([META[agent].name+' returned a live gateway result']);notify(META[agent].name+' completed');loadRuns();refresh();loadCaps();
